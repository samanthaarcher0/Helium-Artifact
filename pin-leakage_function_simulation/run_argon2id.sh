...
DIR="${out_dir}/logs_argon2id_${experiment}_${round}"
...
if [ "$runtime" -eq 1 ]; then
  ...
  for i in $(seq 0 99); do
    if [ "${no_pin:-0}" -eq 1 ]; then
      START=$(date +%s%N)
      ${SCRIPT_DIR}/../workloads/helium_eval_argon2id $pw
      END=$(date +%s%N)
    else
      pw=$(cat /dev/urandom | tr -dc '[:graph:]' | fold -w 100 | head -n 1)
      START=$(date +%s%N)
      "$PIN_ROOT/pin" -t ${SCRIPT_DIR}/obj-intel64/TracerSim.so -start crypto_pwhash  $opts -o $DIR/trace_$i.log -- ${SCRIPT_DIR}/../workloads/helium_eval_argon2id $pw 100
      END=$(date +%s%N)
    fi
