...
DIR="${out_dir}/logs_ed25519_${experiment}_${round}"
...
if [ "$runtime" -eq 1 ]; then
  ...
  for i in $(seq 0 99); do
    if [ "${no_pin:-0}" -eq 1 ]; then
      ...
      START=$(date +%s%N)
      "${PIN_ROOT}/pin" -t "${SCRIPT_DIR}/obj-intel64/TracerSim.so" -start crypto_sign $opts -o "${DIR}/trace_${i}.log" -- "${SCRIPT_DIR}/../workloads/helium_eval_ed25519" "$pw"
      END=$(date +%s%N)
    fi
