...
DIR="${out_dir}/logs_chacha20poly1305_${experiment}_${round}"
...
if [ "$runtime" -eq 1 ]; then
  ...
  for i in $(seq 0 99); do
    if [ "${no_pin:-0}" -eq 1 ]; then
      START=$(date +%s%N)
      "${SCRIPT_DIR}/../workloads/helium_eval_chacha20_poly1305" "$pw"
      END=$(date +%s%N)
    else
      START=$(date +%s%N)
      "$PIN_ROOT/pin" \
        -t "${SCRIPT_DIR}/obj-intel64/TracerSim.so" \
        -start crypto_aead_chacha20poly1305_ietf_encrypt \
        $opts \
        -o "$DIR/trace_$i.log" \
        -- "${SCRIPT_DIR}/../workloads/helium_eval_chacha20_poly1305" "$pw"
      END=$(date +%s%N)
